...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        for a in self.actions_list:
            valid = True
            for c in a.precond_pos:
                if c not in decoded_state.pos:
                    valid = False
                    break
            if valid:
                for c in a.precond_neg:
                    if c not in decoded_state.neg:
                        valid = False
                        break
            if valid:
    ...
    def result(self, state: str, action: Action):
        ...
        for c in old_state.pos:
            if c not in action.effect_rem:
                new_state.pos.append(c)
        for c in old_state.neg:
            if c not in action.effect_add:
                new_state.neg.append(c)

        for c in action.effect_add:
            if c not in new_state.pos:
                new_state.pos.append(c)
        for c in action.effect_rem:
            if c not in new_state.neg:
                new_state.neg.append(c)
