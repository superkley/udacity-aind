...
from competition_agent_old import (CustomPlayerOld)

NUM_MATCHES = 10  # number of matches against each opponent (default: 5)
TIME_LIMIT = 150  # number of milliseconds before timeout (default: 150)
...
def play_round(cpu_agent, test_agents, win_counts, num_matches):
    ...
    for i in range(num_matches):
        ...
        for game in games:
            winner, loser, termination = game.play(time_limit=TIME_LIMIT)
            win_counts[winner] += 1
            # print('cpu: {} winner: {}'.format(cpu_agent.name, winner))
...
def main():
    ...
    test_agents = [
        Agent(CustomPlayer(), "Player")
    ]

    # Define a collection of agents to compete against the test agents
    cpu_agents = [
        Agent(CustomPlayerOld(), "PvP"),
